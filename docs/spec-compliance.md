...
- **S8.6** Unquoted string cannot begin with `0-9` or `-` — §Unquoted strings (L270) *(spec wording is verbatim; the implementation follows the E8 / Lightbend reading — see status note below)*
  ...
  - **Multi-tail extension ([#83](https://github.com/o3co/go.hocon/issues/83))**: signed-numeric adjacent tails (`123-456 = 1` → `{"123-456": 1}`, `123-456abc = 1` → `{"123-456abc": 1}`, `123-456.foo = 1` → path `["123-456", "foo"]`) now converge. The lexer's `readNumber` produces `TokenInt("-456")` when `-` is immediately followed by digits; the parseKey concat loop accepts `TokenInt` / `TokenFloat` tails alongside the existing unquoted / keyword set, and iterates so chains converge (`1.5-2.5 = 1` → path `["1", "5-2", "5"]` via dot-split). Quoted-key gating preserved (`"a.b"-1 = 1` stays rejected). Cross-impl: ts.hocon and rs.hocon read `123-456` as a single unquoted token via their Option B token model, so they handle this naturally — this PR closes go.hocon's Option A divergence.
  tests: internal/parser/parser_test.go (TestSpecS8_6_SignedNumericTailMergedIntoKey, TestSpecS8_6_SignedNumericThenAlphaMergedIntoKey, TestSpecS8_6_SignedNumericDottedTailSplitsKey, TestSpecS8_6_FloatTailMergedIntoKey, TestSpecS8_6_QuotedKeyRejectsAdjacentNumeric).
