...
- **S8.1** Forbidden characters rejected (``$ " { } [ ] : = , + # ` ^ ? ! @ * & \``) and whitespace — §Unquoted strings (L245)
  tests: internal/lexer/lexer_test.go:261 (TestUnquotedStarForbidden); issue68_path_empty_segment_test.go (backtick, value/key/mid-token + quoted-string guard); testdata/hocon/unquoted-forbidden/uf01–uf04 (fixtures)
...
- **S8.6** Unquoted string cannot begin with `0-9` or `-` — §Unquoted strings (L270) *(spec wording is verbatim; the implementation follows the E8 / Lightbend reading — see status note below)*
  tests: s8_unquoted_starts_test.go (xx.hocon fixtures: 29 success — us01–us14, us16, us17–us30 — and 1 error — us15; plus 4 path-rule regressions and 10 E8 explicit assertions); internal/lexer/lexer_test.go (TestSpecS8_6_E8_HyphenStartUnquotedAccepted, post-E8 positive guard)
  status: ✅ (post-E8 amendment) — xx.hocon E8 rewritten 2026-05-20 ([xx.hocon#31](https://github.com/o3co/xx.hocon/issues/31) / commit `dd102e8`) adopts Lightbend's pragmatic reading of HOCON.md L270-276 ("begin" = value-position begin, not token-position). go.hocon now matches:
  - Value-start `-` not followed by a digit → unquoted text (was lex error). RFC 8259 JSON-number requires a digit after `-`; bare `-` / `-foo` therefore fall outside L270's disallow scope. Lightbend reference produces `{"a":"-foo"}` / `{"a":"-"}`. Implemented via the `nextToken` dispatch peek-ahead: `-` followed by a digit dispatches to `readNumber`; otherwise dispatches to `readUnquotedOrKeyword`.
  - Value-start digit-leading runs → normalized via `strconv.ParseInt` at value-position only in `parseSingleValue`'s `TokenInt` case (`internal/parser/parser.go`). `a = 01` → `ScalarNode.Raw = "1"`; `a = -0` → `"0"`; value-concat strings inheriting the integer prefix also normalize (`a = 01s` → `"1s"`). **F3 BREAKING** for `GetString` callers (was raw `"01"` / `"-0"` / `"01s"`); `GetInt64`, `GetFloat64`, JSON serialization, `parseDuration`, `parseBytes` unaffected. Matches Lightbend's `parseLong` behavior. **Important**: normalization is value-position only — the lexer keeps verbatim `TokenInt.Value`, so `parseKey` reading the same token sees raw text and `01 = x` parses under key `"01"` (NOT renamed to `"1"`).
  - Concat-continuation after value-tokens (e.g. `b = ${a}-bar` → `"foo-bar"`) now accepted — removing the strict `-` reject at dispatch lets the unquoted run continue naturally. Symmetric for `"foo"-bar`, `${a}1bar`, `${a}.bar`.
  - `+` rejection retained in both value-start (`a = +foo`) and concat-continuation (`b = ${a}+bar`) positions (HOCON `+=` operator reservation). Pre-E8 go.hocon **accepted** these as unquoted strings (a pre-existing gap vs. ts/rs); the E8 PR closes it by emitting `TokenError` from the bare-`+` dispatch branch (was `readUnquoted("+", ...)`). us15 `a = 1e+x` is now caught by this same gate — the `+` left after exponent-backtrack hits the bare-`+` dispatch and errors — so us15 moved from the previous "known gap" list to the regular error fixtures.
  ...
  - **Out of E8 scope (preserved from #81-followup)**: parser-level numeric-key support gated to numeric-leading keys (us08 `123abc = 1` → `{"123abc": 1}` via TokenInt + unquoted concat, us09 `3.14 = "v"` → `{"3":{"14":"v"}}` via TokenFloat dot-split, keyword-tail keys like `123true = 1` → `{"123true": 1}`). Quoted-key concat like `"a.b"c = 1` remains rejected so the literal `.` is never re-interpreted as a path separator.
  - **Multi-tail extension ([#83](https://github.com/o3co/go.hocon/issues/83))**: signed-numeric adjacent tails (`123-456 = 1` → `{"123-456": 1}`, `123-456abc = 1` → `{"123-456abc": 1}`, `123-456.foo = 1` → path `["123-456", "foo"]`) now converge. The lexer's `readNumber` produces `TokenInt("-456")` when `-` is immediately followed by digits; the parseKey concat loop accepts `TokenInt` / `TokenFloat` tails alongside the existing unquoted / keyword set, and iterates so chains converge (`1.5-2.5 = 1` → path `["1", "5-2", "5"]` via dot-split, `123-456-789 = 1` → `{"123-456-789": 1}`). Quoted-key gating preserved (`"a.b"-1 = 1` stays rejected). Cross-impl: ts.hocon and rs.hocon read `123-456` as a single unquoted token via their Option B token model, so they handle this naturally — this PR closes go.hocon's Option A divergence. Tests: internal/parser/parser_test.go (TestSpecS8_6_*: SignedNumericTailMergedIntoKey, SignedNumericThenAlphaMergedIntoKey, SignedNumericDottedTailSplitsKey, FloatTailMergedIntoKey, TripleSignedNumericChain, MultipleAdjacentTokenInts, TrailingDotInTailContinuesOuterLoop, QuotedKeyRejectsAdjacentNumeric).
...
- **S11.8** Path expression always stringifies (single `true` → `"true"`) — §Path expressions (L504)
  tests: internal/parser/parser_test.go (TestSpecS11_8_* — bool/false/null literal keys, bool key with string value, keyword-prefix is not keyword, quoted/unquoted equivalence, composes with S10.8 space-concat, dotted-path baseline)
